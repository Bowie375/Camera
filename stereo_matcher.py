...
def compute_row_disp_vectorized(Lrow: np.ndarray, Rrow: np.ndarray, mask_row: np.ndarray, max_disp: int, win: int):
    ...
    # Precompute channel sums and squares for L (for all window centers)
    #csL = np.vstack((np.zeros((1, c), dtype=Lrow.dtype), np.cumsum(Lrow, axis=0)))
    #csL2 = np.vstack((np.zeros((1, c), dtype=Lrow.dtype), np.cumsum(Lrow * Lrow, axis=0)))

    # window sum for each center x is cs[x+half+1] - cs[x-half]
    cs_left = xs- half
    cs_right = xs + half + 1

    # Sum over window (for all centers) shape (n_centers, c)
    # sumL_win = csL[cs_right, :] - csL[cs_left, :]
    # sumL2_win = csL2[cs_right, :] - csL2[cs_left, :]

    # For the right image
    if mask_row is None:
        mask_row = np.ones((w, c), dtype=np.uint8)
    else:
        mask_row = np.repeat(mask_row[:, np.newaxis], c, axis=1)

    # Build masked L once: (w,c)
    mask_col = mask_row.astype(Lrow.dtype)
    L_masked = Lrow * mask_col
    R_masked = Rrow * mask_col

    # recompute cs for L_masked
    ...
    csMask = np.concatenate((np.zeros((1,c), dtype=np.int32), np.cumsum(mask_row.astype(np.int32), axis=0)), axis=0)
    count_win = csMask[cs_right, :] - csMask[cs_left, :]  # shape (n_centers, c)
    ...
    n_centers = len(xs)
    best_costs = np.full(n_centers, -1e9, dtype=np.float32)  # store best ncc (we use ncc not cost)
    ...
    for d in range(1, max_d):
        ...
        if d >= w:
            break
        # make shifted R array where positions <0 are zeros
        # instead of allocating full new array for all d, use view slicing
        ...
        # sum over channels
        sumProd = np.sum(sumProd_win, axis=1)  # shape (n_centers,)

        # sums over channels for L and R
        sL = np.sum(sumLm_win, axis=1)   # shape (n_centers,)
        sR = np.sum(sumRm_win, axis=1)
        sL2 = np.sum(sumL2m_win, axis=1)
        sR2 = np.sum(sumR2m_win, axis=1)
        ...
        cnt = count_win.sum(axis=1).astype(np.float32)
        valid = cnt > 0
        ...
        ncc = numer / np.sqrt(denom)

        # keep only centers where xR window fully valid: Note shifting makes leftmost centers invalid for large d.
        # After shifting R by d to the right, centers x with (x - d - half) < 0 had truncated windows; we want to avoid these
        # We can compute allowed centers indices: center x must satisfy x - d - half >= 0  => x >= d + half
        allowed_centers_mask = (xs >= (d + half))  # boolean array of length n_centers
        # apply mask
        ncc_masked = ncc.copy()
        ncc_masked[~allowed_centers_mask] = -1e9  # invalid

        # update best
        better = ncc_masked > best_costs
        if np.any(better):
            best_costs[better] = ncc_masked[better]
            best_disp[better] = d
...
def compute_row_disp_vectorized_1(Lrow: np.ndarray, Rrow: np.ndarray, mask_row: np.ndarray, max_disp: int, win: int):
    """
    Lrow, Rrow: (w, c) float32
    mask_row: (w,) uint8 (0/1). If None, all ones.
    returns disp_row: (w,) float32 with nan where invalid
...
def _worker_unpack_args(args):
    return compute_row_disp_vectorized_1(*args)
